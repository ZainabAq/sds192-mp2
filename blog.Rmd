...
filter_Votes <- function(fieldA, fieldB) {
  newdata <- new_house %>% 
   filter(fieldA > 0) %>%
   filter(fieldB > 0) %>%
   ...
   arrange(desc(diff)) %>%
   top_n(100) 
ggplot(newdata, aes(x = primary_votes, y = general_votes)) + geom_point(aes(color = party)) + scale_color_manual(values = c("blue", "red")) + theme_wsj()
}
filter_Votes(fieldA= "primary_votes", fieldB = "general_votes")
```
